...
from config import (
    ...
    OCR_THRESHOLD, KEEP_FROM_PAGE
)
from utils import (
    setup_logging, is_valid_file, generate_report,
    filter_first_pages, build_record
)
from extractors import (
    is_extraction_allowed, fallback_ocr,
    PyPDFStrategy, PDFMinerStrategy,
    PDFMinerLowLevelStrategy, UnstructuredStrategy, OCRStrategy
)
...
def process_file(path: str, strategy: str, db_choice: str, results: dict):
    ...
    # Filtra primeiras páginas
    text = filter_first_pages(path, KEEP_FROM_PAGE)

    # Se não for possível extrair texto, usa OCR
...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            process_file(path, strat, db_choice, results)

        elif choice == "2":
            folder = input("Caminho da pasta: ").strip()
            if not folder:
                logging.error("Nenhuma pasta informada. Tente novamente.")
                continue
            if not os.path.isdir(folder):
                logging.error(f"Pasta não encontrada: {folder}. Tente novamente.")
                continue
            ...
            with ThreadPoolExecutor() as executor:
                for fname in os.listdir(folder):
                    ...
                    if os.path.isfile(full) and fname.lower().endswith(('.pdf', '.docx')):
                        executor.submit(process_file, full, strat, db_choice, results)
        else:
            logging.warning("Opção inválida. Escolha 1, 2 ou 0.")
