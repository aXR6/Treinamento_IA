...
def choose_strategy():
    choice = input("Estratégia (1-pypdf, 2-pdfminer, 3-unstructured, 4-ocr) [4]: ").strip()
    return {"1":"pypdf","2":"pdfminer","3":"unstructured","4":"ocr"}.get(choice, "ocr")

def process_file(path, strategy, db_choice, results):
    if not is_valid_file(path):
        results["errors"].append(path); return
    # Fallback de OCR se não permitido
